...
async function saveMusicCache(channelId, messageId) {
  await db.from("music_cache").upsert({
...
async function getGuildMusicMonitor(guildId) {
  const { data: guild } = await db
    .from("guilds")
    .select("music_monitor_channel_id, music_monitor_enabled")
    .eq("guild_id", guildId)
    .single();
  return (
    guild || { music_monitor_channel_id: null, music_monitor_enabled: false }
  );
}
...
async function setAutoMod(guildId, settings) {
  const { error } = await db.from("automod_settings").upsert({
    guild_id: guildId,
    ...settings,
    updated_at: new Date().toISOString(),
  });
...
async function setAfk(userId, guildId, reason = null) {
  const { error } = await db.from("afk_status").upsert({
    user_id: userId,
    guild_id: guildId,
    reason,
...
async function addWarning(guildId, userId) {
  ...
  const { error } = await db.from("mod_warnings").upsert({
    guild_id: guildId,
    user_id: userId,
...
async function incrementStat(guildId, statName) {
  ...
  const { error } = await db.from("mod_statistics").upsert({
    guild_id: guildId,
...
async function updateMutualCheckin(user1, user2, guildId, checkingUserId) {
  ...
  const lastMutualCheckDate = mutual.last_mutual_checkin
    ? new Date(mutual.last_mutual_checkin).toISOString().slice(0, 10)
    : null;

  if (lastMutualCheckDate === today && mutual.streak > 0) {
    return {
      success: false,
      reason: "pair_already_checked_today",
      error: "Streak pair ini sudah checked-in hari ini!",
    };
  }
  ...
  const otherCheckinTime = isUser1
    ? mutual.user2_checkin_time
    : mutual.user1_checkin_time;

  const otherCheckedToday =
    otherCheckinTime &&
  ...
  const { error } = await db.from("mutual_streaks").upsert(updates);
...
/**
 * Get the AI config for a guild (channel_id + persona).
...
/**
 * Upsert AI config fields for a guild.
 * Only the keys provided in `updates` are written.
 */
async function setAiConfig(guildId, updates) {
  ...
  if (error) {
    console.error("[Storage] setAiConfig error:", error.message);
    throw error;
  }
}
...
module.exports = {
  ...
  getMusicCache,
  saveMusicCache,
  getGuildMusicMonitor,
  setGuildMusicMonitor,
  disableGuildMusicMonitor,
  getAutoMod,
  setAutoMod,
  setAfk,
  getAfk,
