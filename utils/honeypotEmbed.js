...
function createTriggerEmbed(data) {
  ...
  // Add detected invites if any
  if (detectedInvites && detectedInvites.length > 0) {
    embed.addFields({
      name: '🔗 Detected Invites',
      value: detectedInvites.slice(0, 5).map(inv => `\`${inv}\``).join('\n'),
      inline: false,
    });
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        ...
        value: config.honeypot_channel_id ? `<#${config.honeypot_channel_id}>` : 'Not configured',
        inline: true,
      },
      {
        name: '📋 Log Channel',
      ...
      },
      {
        ...
        value: config.timeout_duration_ms ? formatMs(config.timeout_duration_ms) : 'N/A',
        inline: true,
      },
      {
...
function createTestEmbed(guild) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: '🚨 When Triggered',
        value: 'When someone sends an invite link in the configured honeypot channel, this bot will:\n1. Delete the message\n2. Clean up recent messages from that user\n3. Apply the configured punishment\n4. Log the incident',
      },
      {
        name: '✅ Safe Actions',
